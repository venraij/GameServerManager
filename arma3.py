...
import os, socket, winshell, shutil
...
def arma3Run ():
    ...
    try:
        os.system(os.path.join(os.getcwd(),SERVER_DIR) + "/arma3server.exe" +" -profiles=" + (os.path.join(os.getcwd(),SERVER_DIR)) + " -port=2302 -config=CONFIG_server.cfg -world=empty")
    except:
...
def arma3Install():
    ...
    portArma = input("To which port do you want players to connect? (DEFAULT: 2302): ")
    portQuery = input("The port used by Steam query (DEFAULT: 2303):" )
    portSteam = input("Regular steam port (DEFAULT: 2304) " )
    ...
    try:
        if os.path.exists(os.path.join(os.getcwd(),SERVER_DIR) + '/CONFIG_server.cfg') == False:  
            ...
            with open(os.path.exists(os.path.join(os.getcwd(),SERVER_DIR)), 'CONFIG_server.cfg', 'w') as OPATH:
                OPATH.writelines([
                                    ...
                                    + 'steamPort		= ' + steamPort + ';		// default  8766, needs to be unique if multiple servers are on the same box \n'
                                    + 'steamQueryPort	= ' + portQuery + ';	// default 27016, needs to be unique if multiple servers are on the same box \n'
                                    + '\n'
                                    + '// GENERAL SETTINGS \n'
        ...
        elif os.path.exists(os.path.join(os.getcwd(),SERVER_DIR) + '/CONFIG_server.cfg') == True:
            if (input(bcolors.FAIL + 'The file ' + os.path.join(os.getcwd(),SERVER_DIR) + '/CONFIG_server.cfg already exists. Would you like to override it? (y/n): ' + bcolors.ENDC).lower()) == 'y':
    ...
    try:
        ...
        wDir = r"" + os.path.join(os.getcwd(),SERVER_DIR)
        icon = r"" + os.path.join(os.getcwd(),SERVER_DIR) + "/arma3server.exe"
        args = ["-profiles=" + os.path.join(os.getcwd(),SERVER_DIR),"-port=" + portArma + "","-config=CONFIG_server.cfg","-world=empty"]
